...
const UserGuideContent = ({setCurrentPage}) => {
    const guideSteps = [
    ...
    <div className="max-w-6xl mx-auto">
        <div className="bg-white/45 px-12 rounded-b-2xl min-h-[400px] pt-3">
            <div className="space-y-1">
                ...
                <div className={`flex justify-between items-center ${expandedStep ? 'mt-0' : 'mt-4'}`}>
                    <button onClick={()=>setCurrentPage("home")} className='bg-white/75 hover:bg-white px-8 py-1 rounded-full border'>Back</button>
                    <button onClick={()=>setCurrentPage("retrofit")}  className ='bg-white/75 hover:bg-white px-8 py-1 rounded-full border'>Next</button>
                </div>
            </div>
        </div>
    </div>
